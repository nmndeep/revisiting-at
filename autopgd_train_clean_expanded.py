...
def trades_loss(logits_adv, labels, logits_clean, beta):
    pass
...
def apgd_train(model, x, y, norm, eps, n_iter=10, use_rs=False, loss='ce',
    verbose=False, is_train=True, alpha_init=2., noise_level=1.,
    skip_projection=False, return_last=False, **kwargs):
    ...
    if False:
        print(f'random staart={use_rs} with noise={noise_level}*eps',
            f' alpha_init={alpha_init}, return last={return_last}',
            f' skip projection={skip_projection}',
            f' n_iter={n_iter}')
        #print_once = False
    ...
    if not use_rs:
        ...
        #
        if norm == 'Linf':
            t = (2. * torch.rand_like(x) - 1.) * eps * noise_level
            x_adv = x + t
    ...
    # set loss
    if loss not in ['trades']:
        criterion_indiv = criterion_dict[loss]
    else:
        trades_beta = kwargs.get('trades_beta', 6.)
        with torch.no_grad:
            clean_logits = model(x.detach())
        criterion_indiv = partial(
            trades_loss, clean_logits=clean_logits, beta=trades_beta)
